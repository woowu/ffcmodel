 ...
 * =====
 *
 * ntime: nominal time, to which a metrics set should be associated.
 *
 */
...
function FfcModel()
{
     ...
     * such as 20200101. Daymark was created from nominalTime.
     */
    ...
    const persistMetrics = (devid, ntime, metrics, cb) => {
        ...
        const wrFile = buf => {
            const {pathname, tmpname} = metricsFilename(devid, ntime);
    ...
    const updateLastGoodValue = (devid, metrics, ntime, cb) => {
        const args = [
            'fm:m:lgv:' + devid,
            'timestamp',
            metrics.timestamp,
        ...
        metrics.metrics.forEach(m => {
            args.push('m' + m.id + '_ntime', ntime.valueOf() / 1000);
    ...
    const markTime = (devid, ntime, cb) => {
        const daymark = timeToDaymark(ntime);
    ...
    const saveMetricsAndUpdateIndex =
        (devid, ntime, metrics, cb) => {
            persistMetrics(devid, ntime, metrics,
                (err, newFile) => {
                    if (err) return cb(err);

                    updateLastGoodValue(devid, metrics, ntime, err => {
                        if (err) return cb(err);
                        markTime(devid, ntime, err => {
    ...
    this.putDevMetrics = (devid, ntime, metrics, cb) => {
        saveMetricsAndUpdateIndex(devid, ntime, metrics, cb);
    };
