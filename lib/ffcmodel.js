...
function FfcModel()
{
    ...
    const updateLastGoodValue = (devid, metrics, scheduleTime, cb) => {
        const args = [
            'fm:m:lgv:' + devid,
            'timestamp',
            metrics.timestamp,
    ...
    const saveMetricsAndUpdateIndex
        = (devid, scheduleTime, realTime, metrics, cb) => {

        persistMetrics(devid, scheduleTime, metrics,
            (err, newFile) => {
                if (err) return cb(err);

                updateLastGoodValue(devid, metrics, scheduleTime, err => {
                    if (err) return cb(err);
                    markTime(devid, scheduleTime, err => {
    ...
    this.putDevMetrics = (devid, scheduleTime, realTime, metrics, cb) => {
        saveMetricsAndUpdateIndex(devid, scheduleTime, realTime, metrics,
            err => {
                if (err) return cb(err)
