...
function FfcModel()
{
    ...
    const blockdir = blockindex => path.join(devDataRoot, blockindex.toString());
    const devBlockdir
        = (devid, blockindex) => path.join(blockdir(blockindex), devid.toString());
    const archiveDir = () => path.join(devDataRoot, 'archive');
    ...
    const getBlocksBackwards = (devid, time, limit, cb) => {
    ...
    const openDeviceBlock = (devid, blockindex, cb) => {
        const dir = devBlockdir(devid, blockindex);
    ...
    const getMinTimeInBlock = (devid, block, cb) =>{
        openDeviceBlock(devid, block, (err, dir, files) => {
            if (err) return cb(err);
    ...
    const getDeviceBlockSpan = (devid, cb) => {
    ...
    this.projectMetrics = (devid, time, metricIdList, cb) => {
        ...
        logger.debug(metricIdList);
        var resultMetricList = [];

        getBlocksBackwards(devid, time, blocksToTravelMax, (err, blockList) => {
            if (err) return cb(err);

            (function walkBlockList(list) {
                if (! list.length) return cb(null, resultMetricList);
                const b = list[0];
                openDeviceBlock(devid, b, (err, dir, files) => {
                    ...
                    projectMetricsFromSortedFiles(dir, files,
                        ...
                        (err, improvedResult) => {
                            ...
                            if (improvedResult.length >= metricIdList)
                                return cb(null, resultMetricList);
                            else
    ...
    this.getDeviceTimeSpan = (devid, cb) => {
        getDeviceBlockSpan(devid, (err, minBlock, maxBlock) => {
            if (err) return cb(err);
            cb(null, minBlock, maxBlock);
            openDeviceBlock(devid, maxBlock, (err, dir, files) => {
            });
        });
    };
    ...
    return (function (ffcModel) {
        ...
        blocksToTravelMax = (2 * 24) / blockHours;
