...
function FfcModel()
{
    ...
    const openDeviceBlock = (devid, blockindex, isArchive, cb) => {
    ...
    const filterAndSortFileList = (fileList, greatestTime) => {
        var timeEpoch;
        if (greatestTime) timeEpoch = Math.trunc(greatestTime.valueOf() / 1000);
    ...
    this.projectMetrics = (devid, time, metricIdList, cb) => {
        ...
        getBlocksBackwards(devid, time, blocksToTravelMax, (err, blockList) => {
            ...
            (function walkBlockList(list) {
                ...
                const b = list[0];
                openDeviceBlock(devid, b, false, (err, dir, files) => {
