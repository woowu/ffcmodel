 ...
 * ====================
 *
 * fm:tm                sorted set of blocks in which we have data
 * fm:m                 sorted set of devid's
 * fm:m:tm:<devid>      sorted set of blocks in which the device have data
 * fm:m:lgv:<devid>     copy of last good value of the device
 *
 * fm:a:tm              sorted set of blocks in which we have archived data
 * fm:a:m:tm:<devid>    sorted set of blocks in which the dev have archived data
 *
 */
...
function FfcModel()
{
    ...
    /**
     * A blockmark is an unambiguous integer identity of a block. Blockmark was
     * created from ticktime. Sample example blockmarks are: 2020010100,
     * 2020010102
     */
    const timeToBlockmark = time => {
    ...
    const blockdir = blockmark => path.join(dataRoot, blockmark.toString());
    const devBlockdir
        = (devid, blockmark) => path.join(blockdir(blockmark), devid.toString());
    const archiveDir = () => path.join(dataRoot, 'archive');

    const devStateFilename = (devid, time) => {
        const dir = devBlockdir(devid, timeToBlockmark(time));
    ...
    const markTime = (devid, ticktime, cb) => {
        const blockmark = timeToBlockmark(ticktime);

        client.zadd(['fm:m:tm:' + devid, blockmark, blockmark], err => {
            if (err) return cb(err);
            client.zadd(['fm:tm', blockmark, blockmark], cb);
        });
    };

    const markArchivedTime = (devid, blockmark, cb) => {
        client.zadd(['fm:a:m:tm:' + devid, blockmark, blockmark], err => {
            if (err) return cb(err);
            client.zadd(['fm:a:tm', blockmark, blockmark], cb);
        });
    };

    const removeBlockIndex = (blockmark, cb) => {
        const walkDevices = (list, removedDevices, cb) => {
            const devid = list.pop();
            if (! devid) return cb(null);
            client.zrem(['fm:m:tm:' + devid, blockmark], (err, reply) => {
                if (err) return cb(err);
        ...
        client.zrem(['fm:tm', blockmark], err => {
    ...
    const removeBlock = (blockmark, cb) => {
        logger.info('remove block ' + blockmark);
        removeBlockIndex(blockmark, (err, removedDevices) => {
            if (err) return cb(err);

            try {
                shell.rm('-rf', blockdir(blockmark));
            } catch (err) {
                return cb(err);
            }
            cb(null, removedDevices);
    ...
    /**
     * Remove any block greater than the block indicated by the given (blockmark,
     * score).
     */
    const removeBlocksAfter = (blockmark, cb) => {
        const backWalkBlockList = (list, cb) => {
            const blockmark = list.pop();
            if (! blockmark) return cb(null);

            removeBlock(blockmark, err => {
        ...
        client.zrangebyscore(['fm:tm', '(' + blockmark, '+inf'], (err, reply) => {
            if (err || ! reply || ! reply.length) return cb(err);
            backWalkBlockList(reply, cb);
        });
    };

    const archiveBlock = (blockmark, cb) => {
        logger.info('archive block ' + blockmark);

        const sdir = path.basename(blockdir(blockmark));
        const pdir = path.dirname(blockdir(blockmark));
        const archiveName = path.join(archiveDir(), blockmark + '.tgz');
        ...
        try {
            ...
            shell.exec(cmdline, { silent: true }, (code, stdout, stderr) => {
                ...
                removeBlock(blockmark, (err, removedDevices) => {
                    if (err) return cb(err);
                    (function markArchivedTimeFor(devices, cb) {
                        const m = devices.pop();
                        if (! m) return cb(null);
                        markArchivedTime(m, blockmark, err => {
                            if (err) return cb(err);
    ...
    const archiveAgedBlocks = (level1Blocks, cb) => {
        ...
        client.zrange(['fm:tm', 0, -1], (err, blockList) => {
    ...
    this.housekeeping = (options, cb) => {
        removeBlocksAfter(timeToBlockmark(new Date()), err => {
            if (err) return cb(err);
